<template>
  <div @click="wang" class="hello">
      网上练兵
    <div >{{fangXiang[6]}} {{leibie[0]}} <button @click=" getHouTaiData()">开始</button>  <button @click="wang(0,1)">结束</button></div>
    <button @click="getData(0)">导出表格</button> 
       ...
       setTimeout( ()=> {
         ...
         axios.get("/cglb/ajaxdt?passid=291"+"&id="+_this.tiAllInformation.id+"&answer="+_this.tiAllInformation.answer).then( (res) => {
              ...
              _this.yiChangError =false;  // 控制异常
               ...
               if(res.data.isright == 1) {  // 如果这道题对的话 进行替换题目
